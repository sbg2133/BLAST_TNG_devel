...
class roachInterface(object):
    ...
    def __init__(self):
        ...
        self.gbe_fpga = corr.katcp_wrapper.FpgaClient(self.ip)
	self.fpga = casperfpga.katcp_fpga.KatcpFpga(self.ip,timeout=120.)
        ...
        self.test_comb = np.linspace(-200.0e6, 200.0e6, 500)
        ...
        #self.fpga.write_int('rx_ack', 0)
        
        self.accum_len = (2**20)-1 # 2**20 - 1 for 244 Hz 
    ...
    def calculate_amps_new(self,save=True):
        ...
        save_path='/mnt/iqstream/'
        fit_params=np.empty([4,len(channels)])
        fit_params_I=np.empty([4,len(channels)])
        fit_params_Q=np.empty([4,len(channels)])
        for chan in range(len(channels)):
            fit_params[:,chan] = np.polyfit(sweep_freqs + bb_freqs[chan],10.*np.log10(np.sqrt(Is[:,chan]**2+Qs[:,chan]**2)),3)
            fit_params_I[:,chan] = np.polyfit(sweep_freqs + bb_freqs[chan],Is[:,chan],3)
            fit_params_Q[:,chan] = np.polyfit(sweep_freqs + bb_freqs[chan],Qs[:,chan],3)
        if save:
            np.save(os.path.join(save_path,'fit_params.npy'),fit_params)
            np.save(os.path.join(save_path,'fit_params_I.npy'),fit_params_I)
            np.save(os.path.join(save_path,'fit_params_Q.npy'),fit_params_Q)
        return fit_params    
    ...
    def calc_plot_new(self,save=True):
        fit_params = self.calculate_amps_new(save)
        bb_freqs = np.load('/mnt/iqstream/last_bb_freqs.npy')
        channels = np.load('/mnt/iqstream/last_channels.npy')
        lo_freqs, Is, Qs = self.open_stored(save_path = np.load('/mnt/iqstream/last_vna_dir.npy')[0])
        fig,axs = plt.subplots(2,1)
        for chan in range(len(channels)):
            p=np.poly1d(fit_params[:,chan])
            axs[0].plot((lo_freqs + bb_freqs[chan])/1.0e9,10*np.log10(np.sqrt(Is[:,chan]**2+Qs[:,chan]**2))/p(lo_freqs + bb_freqs[chan]),label='loopback/polyfit')
            axs[1].plot((lo_freqs + bb_freqs[chan])/1.0e9,p(lo_freqs + bb_freqs[chan]),label='polyfit')
            axs[1].plot((lo_freqs + bb_freqs[chan])/1.0e9,10*np.log10(np.sqrt(Is[:,chan]**2+Qs[:,chan]**2)),label='loopback')
        #axs[1].legend(loc = 'lower left', fontsize = 15)
        plt.grid()            
        if save:
            plt.savefig('/mnt/iqstream/CalVNAloopback.png', bbox = 'tight') 
        plt.show()
        return
...
# Builds the DDS look-up-table from I and Q given by freq_comb. freq_comb is called with the sample rate equal to the sample rate for a single FFT bin. There are two bins returned for every fpga clock, so the bin sample rate is 256 MHz / half the fft length  
    ...
    # packs the I and Q look-up-tables into strings of 16-b integers, in preparation to write to the QDR. Returns the string-packed look-up-tables
        self.I_dac, self.Q_dac = self.freq_comb(freqs, self.dac_samp_freq, self.dac_freq_res, amplitudes, random_phase = True)
    ...
    def target_sweep(self, save_path = '/mnt/iqstream/target_sweeps', write = True, span = 350.0e3):
        write = raw_input('Write ? (y/n) ')
        kid_freqs = np.load('/mnt/iqstream/lightfreqs.npy')
    ...
    def find_peaks(self, path, bb_freqs, plot_chan = False, plot_sweep = False):
	...
	for chan in channels:
		...
		peaks = detect_peaks(chan_data, mph = thresh, mpd = np.round(100.0e3/2.5e3))
		if len(peaks) > 0:
			for peak in peaks:
    ...
    def vna_sweep(self, center_freq = 750.0e6, save_path = '/mnt/iqstream/vna_sweeps', write = True, offset = False, find_peaks = False, find_peaks_edges = False):
        ...
        #calc = raw_input('Use Cal file? (y/n)')
        sweep_dir = raw_input('VNA sweep dir ? ')
        save_path = os.path.join(save_path, sweep_dir)
        bb_freqs, delta_f = np.linspace(-200.0e6, 200.0e6, 500,retstep=True)
	bb_save_path = '/mnt/iqstream/bb_freqs.npy'
	if offset:
		bb_freqs, delta_f = np.linspace(-200.0e6, 200.0e6, 500,retstep=True) + 50.0e3
		bb_save_path = '/mnt/iqstream/bb_freqs_offset.npy'
        ...
        #self.plot_kids(save_path = last_vna_dir, bb_freqs = bb_freqs, channels = channels, calc=calc)
        if find_peaks:
		peak_idx, chan_freqs, scaled_mags = self.find_peaks(self, save_path, bb_freqs, plot_chan = False, plot_sweep = False)
	if find_peaks_edges:
		peak_idx_1, chan_freqs_1, scaled_mags_1 = self.find_peaks(self, save_path, bb_freqs, plot_chan = False, plot_sweep = False)
		peak_idx_2, chan_freqs_2, scaled_mags_2 = self.find_peaks(self, save_path_offset, bb_freqs, plot_chan = False, plot_sweep = False)
		#kid_freqs_1 = chan_freqs_1(peak_idx_1)
		#kid_freqs_2 = chan_freqs_2(peak_idx_2)
		
	self.kid_freqs = chan_freqs[peak_idx]
	return 

    def generate_vna_cal(self, center_freq = 750.0e6, save_path = '/mnt/iqstream/vna_sweeps', write = True):
        garbage = raw_input('Is the system in loopback mode?')
        write = raw_input('Write tones and sweep? (y/n)')
        sweep_dir = raw_input('VNA sweep dir ? ')
        save_path = os.path.join(save_path, sweep_dir)
	print save_path
        bb_freqs, delta_f = np.linspace(-200.0e6, 200.0e6, 500,retstep=True)
        ...
        if write=='y':
            ...
            self.sweep_lo(Npackets_per = 100, channels = channels, center_freq = center_freq, span = delta_f, save_path = save_path)
	    np.save('/mnt/iqstream/last_vna_dir.npy',np.array([save_path]))
            save_path = np.load('/mnt/iqstream/last_vna_dir.npy')
	    print save_path[0]
	    self.calc_plot_new(save=True)    
        else:
    ...
    def plot_kids(self, save_path = None, bb_freqs = None, channels = None, calc=True):
    ...
    def find_kids_vna(self,save_path=None,usecalc=True):
        ...
        if usecalc:
            fit_params_I=np.load('/mnt/iqstream/fit_params_I.npy')
            fit_params_Q=np.load('/mnt/iqstream/fit_params_Q.npy')
            for chan in np.arange(len(sweep_freqs)):
                p=np.poly1d(fit_params_I[:,chan])
                c=np.poly1d(fit_params_Q[:,chan])
                Is[:,chan]=Is[:,chan]/p(sweep_freqs + bb_freqs[chan]) #normalizes the Is to +1/-1
                Qs[:,chan]=Qs[:,chan]/c(sweep_freqs + bb_freqs[chan]) #normalizes the Qs to +1/-1
    ...
    def main_opt(self):
        while True:
            ...
            if opt == 0:
                ...
                self.initialize() 
            if opt == 1:
                print '\nTest tone (MHz) =', self.test_freq/1.0000e6
            ...
            if opt == 4:
                self.vna_sweep(find_peaks = True)
            if opt == 5:
                self.find_kids_vna()
            if opt == 6:
                self.target_sweep()
